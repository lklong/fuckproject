...
<body >
<!-- //功能条// -->
<form method="post" id="memberForm" action="admin/member/commonMember" class="fl">
...
<div class="tableActBar">
    ...
    <!-- <a href="javascript:void(0)" class="sysButonA2"><em></em>批量发送消息</a> --></div>
</div>

        	<table class="sysCommonTable _memTable"  border="0" cellspacing="0" cellpadding="0">
             	<thead>
            	<tr>
...
<div id="recommendUserDiv" style="display:none;">
	<p style="width:660px;text-align:left;font-size:14px;font-weight:bold;line-height:40px;border-bottom:1px dotted #dedede;text-indent:15px;margin:0 20px;"><font color="#ff0000"><span id="lookRecommendUser"></span></font>推荐的已注册用户：</p>
	<div class="ztree"  id="recommendUserTree" style="padding:10px;margin:0 20px;">
		加载用户文件夹...
	</div>
	<br/>
	<div id="userWriteRecommend" style="text-align:center;padding:10px;"></div>
</div>
<!--------------------------------------------------footer---------------------------------------------------------->  
...
zhigu.pageAjaxSuccess = function(html){
	$("#userWriteRecommend").html(html);
}
...
$(function(){
	...
	if(index<0){
		$("#memberForm").prop("action",url);
	}else{
		$("#memberForm").prop("action",url.substring(0,index));
	}
})
