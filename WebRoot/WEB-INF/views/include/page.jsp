...
<form id="_pageForm" action="" method="post">
	...
	<div style="line-height: 35px;">
		&nbsp;&nbsp;&nbsp;总计<b>${page.totalRow }</b>条&nbsp;&nbsp;&nbsp;第<b>${page.pageNo }</b>页
		...
		<a href="javascript:page_go(1);">首页</a>
		<a href="javascript:page_go(${(page.pageNo - 1) < 1 ? 1 : (page.pageNo - 1) });" class="prev" style="width: 50px;">上一页</a>
		<c:if test="${begin != 1 }">
			<font>···</font>
		</c:if>
		<c:if test="${begin == 1 }">
		...
		<c:forEach begin="${begin }" end="${end }" var="p">
			<c:choose>
				<c:when test="${p == page.pageNo }">
					<a href="javascript:void(0);" class="pageselect">${p }</a>
				</c:when>
				<c:otherwise>
					<a href="javascript:page_go(${p });">${p }</a>
				</c:otherwise>
			</c:choose>
		</c:forEach>
		<c:choose>
		...
		<a href="javascript:page_go(${page.totalPage });">尾页</a>
	</div>
</form>
