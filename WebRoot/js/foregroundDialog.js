...
zhigu.confirmDialog = function(options){
	...
	confirmDialog(params.content,params.okFunction,params.cancelFunction,null,params.width,params.height,params.title,params);
}
// 旧的dialog
dialog = function(content, closeFun, width, heigth, title,showBut){
...
(function(exports){
		...
		//创建对话框
		exports.create = function createDialog(_html,_function,_cancel,_close,_width,_height,_title,opt)
		{
...
(function(exports){
		...
		function createCloseBtn(_close)
		{
			...
			if(typeof _close == 'function')
			{
				closeBtn.bind('click',function(){
					zhigu.log("关闭按钮点击");
						_close();
						// 2015/03/07 窗口不能关闭 ADD START
						parentBox.remove();
						maskLayer.destroy();
						// 2015/03/07 窗口不能关闭 ADD END
					});
			}
			else
			{
		...
		function createDoneBtn(_function)
		{
			...
			buttonBox.append(doneBtn);
			// 2015/03/07 窗口不能关闭 UPDATE START
			doneBtn.bind('click',function(){
				if(typeof _function == 'function'){
					_function();
				}
...
function confirmDialog(_html,_function,_cancel,_close,_width,_height,_title,opt)
{
	if(!maskLayer.display)
	{
		maskLayer.create();
	...
	dialogLayer.create(_html,_function,_cancel,_close,_width,_height,_title,opt);
}
