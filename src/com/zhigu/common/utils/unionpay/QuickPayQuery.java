...
/**
 * 名称：商户查询类 功能： 类属性：方法调用 版本：1.0 日期：2011-03-11 作者：中国银联UPOP团队 版权：中国银联
	...
	public void merBusiness(String[] arr) {
		...
		for (int i = 0; i < arr.length; i++) {
			...
			if (queryResultArr.length == 2 && "queryResult".equals(queryResultArr[0])) {
				if ("0".equals(queryResultArr[1])) {
					System.out.println("成功");
				}
				if ("1".equals(queryResultArr[1])) {
					System.out.println("失败");
				}
				if ("2".equals(queryResultArr[1])) {
					System.out.println("处理中");
				}
				if ("3".equals(queryResultArr[1])) {
