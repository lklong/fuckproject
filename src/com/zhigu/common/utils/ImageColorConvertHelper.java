...
public class ImageColorConvertHelper {
	...
	/*
	 * static { JAIRegisterHelper.register(); }
	 */
	...
	public static PlanarImage generalColorConvert(PlanarImage src) {
		...
		ColorModel sRGBColorModel = RasterFactory.createComponentColorModel(DataBuffer.TYPE_BYTE, sRGBColorSpace, false, false, Transparency.OPAQUE);
	...
	public static PlanarImage convertCMYK2RGB(PlanarImage src) {
		...
		/**
		 * ICC_ColorSpace object mean jai read ColorSpace from image embed
		 * profile, we can not inverted cmyk color, and can not repace
		 * BufferedImage's ColorSpace
		 */
		if (srcColorSpace instanceof ICC_ColorSpace) {
			// -- Convert CMYK to RGB
			ColorSpace rgbColorSpace = ColorSpace.getInstance(ColorSpace.CS_sRGB);
			ColorModel rgbColorModel = RasterFactory.createComponentColorModel(DataBuffer.TYPE_BYTE, rgbColorSpace, false, true, Transparency.OPAQUE);
			ImageLayout rgbImageLayout = new ImageLayout();
			...
			ColorModel cmykColorModel = RasterFactory.createComponentColorModel(src.getSampleModel().getDataType(), cmykColorSpace, false, true, Transparency.OPAQUE);
	...
	public static PlanarImage convertRGBA2RGB(PlanarImage src) {
		...
		if (cm.getColorSpace().getType() == ColorSpace.TYPE_RGB && src.getNumBands() == 4) {
			...
			ColorModel cmRGB = new ComponentColorModel(sRGB, bitsRGB, false, false, Transparency.OPAQUE, DataBuffer.TYPE_BYTE);
