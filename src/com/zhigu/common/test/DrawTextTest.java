...
public class DrawTextTest extends TestCase {
	...
	static File path = new File("F:/static/081");
	static File rpath = new File("F:/static/081/draw");
	...
	protected String getFileStr() throws IOException {
		return path.getCanonicalPath() + File.separator + "270.jpg";
	}
	...
	public void testCornerDrawTextItem() throws Exception {
		...
		doDrawImageText("270.jpg", "corner-", item);
		// doDrawImageText("1.jpg", "corner-", item);
		// doDrawImageText("a.jpg", "corner-", item);
		// doDrawImageText("2-1.jpg", "corner-", item);
	}

	@Test
	public void testFixDrawTextItem() throws Exception {
		FixDrawTextItem item = new FixDrawTextItem("远华贸易实业有限公司", new Color(255, 255, 255, 115), new Color(170, 170, 170, 77), FontManager.getFont("方正黑体"), 10, FixDrawTextItem.Position.BOTTOM_RIGHT,
				0.5f);
		doDrawImageText("270.jpg", "corner-", item);
	...
	public void testFootnoteDrawTextItem() throws Exception {
		...
		doDrawImageText("a.jpg", "footnote-", item);
	...
	public void doDrawImageText(String imageName, String desc, DrawTextItem... items) throws Exception {
		...
		try {
			...
			DrawTextRender dtr = new DrawTextRender(in, dp);

			fr = new WriteRender(dtr, new File(rpath, "12.jpg"));
			fr.render();
		} finally {
