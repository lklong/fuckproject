...
import com.zhigu.common.utils.taobao.TaobaoUtil2;
...
public class TaobaoGoodsController2 {

	private static final Logger LOGGER = LoggerFactory.getLogger(TaobaoGoodsController2.class);
	...
	@Autowired
	private TaobaoUtil2 taobaoUtil2;
	...
	public String publishToTaobao(String goodsId, RedirectAttributes redirectAttributes, HttpSession session) {
		...
		if (taobaoToken != null && StringUtils.isNotBlank(taobaoToken.getAccessToken())) {
			...
			if (userInfo != null) {
				...
				redirectAttributes.addAttribute("access_token", access_token);
	...
	public String initPublishItem(String access_token, String code, HttpSession session, ModelMap model) throws IOException, ApiException {
		...
		Integer goodsId = Integer.valueOf(_goodsId.toString());
		...
		if (StringUtils.isBlank(access_token)) {
			...
			try {
				...
				access_token = taobaoUtil2.getToken(code, userId);
