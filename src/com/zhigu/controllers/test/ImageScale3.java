...
public class ImageScale3 {
	...
	public static void imageCompress(String path, String fileName, String toFileName, Float scale, float quality, int width, int height) {
		try {
			...
			if (scale == null || scale > 0.5) {
				scale = 0.5f;// 默认压缩比为0.5，压缩比越大，对内存越高，可能导致内存溢出
			}
			// 按比例计算出来的压缩比
	...
	public static void main(String[] args) throws IOException {
		String folder = "E:/logs/";
		String srcFile = "E:/logs/ce8.jpg";
		...
		FileOutputStream out = new FileOutputStream(folder + "22.jpg");
		JPEGImageEncoder encoder = JPEGCodec.createJPEGEncoder(out);
		JPEGEncodeParam param = encoder.getDefaultJPEGEncodeParam(mBufferedImage);
